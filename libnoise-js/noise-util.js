class NoiseUtil {
    ...
    static X_NOISE_GEN = 1619;
    static Y_NOISE_GEN = 31337;
    static Z_NOISE_GEN = 6971;
    static SEED_NOISE_GEN = 1013;
    static SHIFT_NOISE_GEN = 8;
    static NoiseQuality = Object.freeze({
        QUALITY_FAST: Symbol("QUALITY_FAST"),
        QUALITY_STD: Symbol("QUALITY_STD"),
        QUALITY_BEST: Symbol("QUALITY_BEST"),
    });

    static RandomVectors = [
    ...
    static CubicInterp(n0, n1, n2, n3, a) {
        var p = (n3 - n2) - (n0 - n1);
        var q = (n0 - n1) - p;
        var r = n2 - n0;
        var s = n1;
    ...
    static GradientCoherentNoise3D(x, y, z, seed, noiseQuality) {
        ...
        var xs = 0, ys = 0, zs = 0;
        switch (noiseQuality) {
            case NoiseUtil.NoiseQuality.QUALITY_FAST:
                xs = (x - x0);
                ys = (y - y0);
                zs = (z - z0);
                break;
            case NoiseUtil.NoiseQuality.QUALITY_STD:
                xs = NoiseUtil.SCurve3(x - x0);
                ys = NoiseUtil.SCurve3(y - y0);
                zs = NoiseUtil.SCurve3(z - z0);
                break;
            case NoiseUtil.NoiseQuality.QUALITY_BEST:
                xs = NoiseUtil.SCurve5(x - x0);
                ys = NoiseUtil.SCurve5(y - y0);
                zs = NoiseUtil.SCurve5(z - z0);
                break;
        }

        var n0, n1, ix0, ix1, iy0, iy1;
        n0 = NoiseUtil.GradientNoise3D(x, y, z, x0, y0, z0, seed);
        n1 = NoiseUtil.GradientNoise3D(x, y, z, x1, y0, z0, seed);
        ix0 = NoiseUtil.LinearInterp(n0, n1, xs);
        n0 = NoiseUtil.GradientNoise3D(x, y, z, x0, y1, z0, seed);
        n1 = NoiseUtil.GradientNoise3D(x, y, z, x1, y1, z0, seed);
        ix1 = NoiseUtil.LinearInterp(n0, n1, xs);
        iy0 = NoiseUtil.LinearInterp(ix0, ix1, ys);
        n0 = NoiseUtil.GradientNoise3D(x, y, z, x0, y0, z1, seed);
        n1 = NoiseUtil.GradientNoise3D(x, y, z, x1, y0, z1, seed);
        ix0 = NoiseUtil.LinearInterp(n0, n1, xs);
        n0 = NoiseUtil.GradientNoise3D(x, y, z, x0, y1, z1, seed);
        n1 = NoiseUtil.GradientNoise3D(x, y, z, x1, y1, z1, seed);
        ix1 = NoiseUtil.LinearInterp(n0, n1, xs);
        iy1 = NoiseUtil.LinearInterp(ix0, ix1, ys);

        return NoiseUtil.LinearInterp(iy0, iy1, zs);
    }

    static GradientNoise3D(fx, fy, fz, ix, iy, iz, seed) {
        // Randomly generate a gradient vector given the integer coordinates of the
        // input value.  This implementation generates a random number and uses it
        // as an index into a normalized-vector lookup table.
        var vectorIndex = (
            NoiseUtil.X_NOISE_GEN * ix
            + NoiseUtil.Y_NOISE_GEN * iy
            + NoiseUtil.Z_NOISE_GEN * iz
            + NoiseUtil.SEED_NOISE_GEN * seed) & 0xffffffff;
        //FIXME : Check this!
        // vectorIndex ^= (vectorIndex >> (int)SHIFT_NOISE_GEN);
        vectorIndex ^= (vectorIndex >> NoiseUtil.SHIFT_NOISE_GEN);
        vectorIndex &= 0xff;

        var xvGradient = NoiseUtil.RandomVectors[(vectorIndex << 2)];
        var yvGradient = NoiseUtil.RandomVectors[(vectorIndex << 2) + 1];
        var zvGradient = NoiseUtil.RandomVectors[(vectorIndex << 2) + 2];
    ...
    static IntValueNoise3D(x, y, z, seed) {
    ...
    static ValueNoise3D(x, y, z, seed = 0) {
